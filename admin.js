 (async function () {
  ...
  // Load actual processes from index.html (single source of truth)
  // This avoids mismatches between Admin settings and the Training page.
  let ALL_PROCESSES = [];

  async function loadProcessesFromIndex() {
    try {
      const res = await fetch("index.html", { cache: "no-store" });
      if (!res.ok) throw new Error("Failed to load index.html");
      const html = await res.text();

      const doc = new DOMParser().parseFromString(html, "text/html");
      const sections = Array.from(doc.querySelectorAll(".docSection[data-process-id]"));

      const seen = new Set();
      const list = [];

      sections.forEach((sec) => {
        const id = (sec.getAttribute("data-process-id") || "").trim();
      ...
      console.error("Admin: unable to load processes from index.html", err);
      return [];
    }
  }
  ...
  // Init
  ALL_PROCESSES = await loadProcessesFromIndex();
  if (!ALL_PROCESSES.length) {
    // Fallback: still allow admin page to load if index.html fetch fails.
    ALL_PROCESSES = [{ id: "Welcome", title: "Welcome" }];
  }
